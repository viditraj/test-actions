      ...
      if(context.path.endsWith('.spec.ts')){
        ...
        const transformedContent = content.replace(/import\s+['"]([^'"]+)['"];/g, (match, p1) => {
          ...
          if (isCommonUtils || !isRelativeImport) {
            const absolutePath = isCommonUtils ? resolve(__dirname, "./mfe-template/", p1) : path.resolve(path.dirname(context.path), p1);
...
export default {
    ...
    files: ['mfe-template/**/**/*.spec.ts', '!**/node_modules/**'],
    setupFiles: ['mfe-template/common-utils/jasmine-setup.ts'],
    nodeResolve: true,
